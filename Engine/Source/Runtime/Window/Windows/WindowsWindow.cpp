...
{
    m_props.Title = title;
    SetWindowTextW(m_hWnd, Convert::ToNativeString(m_props.Title).Data());
}
...
LRESULT WindowsWindow::HandleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        // 更新位置属性
        m_props.PositionX = (int16)LOWORD(lParam);
        m_props.PositionY = (int16)HIWORD(lParam);
